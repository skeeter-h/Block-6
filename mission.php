...
<div class="w3-content" style="max-width:2000px;margin-top:46px">
  ...
  <h2>Here are the missions currently in the data system: </h2>
		<hr>
		...
		<table>
            ...
            <tr> <!--Each attribute is presented as a column and the table tag is used to appropriately structur-->
                ...
                <th width="250px">Mission Aim<br><hr></th>
                <th width="250px">Crew Size<br><hr></th>
                <th width="250px">Mission Life (years) <br><hr></th>
                <th width="250px">Site<br><hr></th>
                <th width="250px">Launcher<br><hr></th>
                <th width="250px">Prime Contactor<br><hr></th>
			      </tr>
            ...
            //This code is taken from: https://stackoverflow.com/questions/70714833/php-delete-button-for-every-row-change-value-in-table-for-this-row  
             ...
             //First a connection is established, ensuring that all the necessary fields (servername, username, password and dataase are specified)
                ...
                $sql = "SELECT mission_id, astronaut_id, target_id, mission_name, destination, launch_date, mission_type, mission_aim, crew_size, mission_life, mission_site, launcher, prime_contractor  FROM mission"; 
                ...
                while($row = $result->fetch_assoc()) { //For every existing row, data in the different attributes (from the selected ones) will be fetched as an array of data 
                    ...
                    <th>{$row['mission_aim']}</th>
                    <th>{$row['crew_size']}</th>
                    <th>{$row['mission_life']}</th>
                    <th>{$row['mission_site']}</th>
                    <th>{$row['launcher']}</th>
                    <th>{$row['prime_contractor']}</th>
      ...
      <div class="w3-row-padding w3-padding-32" style="margin:0 -16px">
        <div class="w3-third w3-margin-bottom">
          <div class="w3-container w3-white">
            <p><b>Add a new mission to the database</b></p>
            <button class="w3-button w3-black w3-margin-bottom" onclick="window.location.href='#show-table'">MODIFY</button>
          </div>
        </div>
        <div class="w3-third w3-margin-bottom">
          <div class="w3-container w3-white">
            <p><b>Delete an existing mission from the database</b></p>
            <button class="w3-button w3-black w3-margin-bottom" onclick="window.location.href='Deletemission.php'">DELETE</button> 
          </div>
        </div>
        <div class="w3-third w3-margin-bottom">
          <div class="w3-container w3-white">
            <p><b>Edit the details of an existing mission</b></p>
            <button class="w3-button w3-black w3-margin-bottom" onclick="window.location.href='Addmission.php'">INSERT</button> 
          </div>
        </div>
      </div>
    </div>
